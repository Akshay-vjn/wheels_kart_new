...
class DashedBorderPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    Paint paint = Paint()
      ..color = AppColors.kGrey // Border color
      ..strokeWidth = 2
    ...
    drawDashedLine(canvas, const Offset(0, 0), Offset(size.width, 0), dashWidth,
        dashSpace, paint);
    // Draw dashed right border
    drawDashedLine(canvas, Offset(size.width, 0),
        Offset(size.width, size.height), dashWidth, dashSpace, paint);
    // Draw dashed bottom border
    drawDashedLine(canvas, Offset(size.width, size.height),
        Offset(0, size.height), dashWidth, dashSpace, paint);
    // Draw dashed left border
    drawDashedLine(canvas, Offset(0, size.height), const Offset(0, 0),
        dashWidth, dashSpace, paint);
  }

  void drawDashedLine(Canvas canvas, Offset start, Offset end, double dashWidth,
