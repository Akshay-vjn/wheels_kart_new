...
class VProfileRepo {
  static Future<Map<String, dynamic>> onFetchProfile(
      BuildContext context) async {
    final state = context.read<AppAuthController>().state;
    if (state is AuthCubitAuthenticateState) {
      try {
        final url = Uri.parse('${VApiConst.baseUrl}${VApiConst.profile}');
        ...
        log('repo - catch error - vendor - PROFILE => ${e.toString()}F');
        return {};
      }
    } else {
      return {};
