...
import 'package:wheels_kart/module/EVALAUATOR/data/model/system_model.dart';
import 'package:wheels_kart/module/EVALAUATOR/data/repositories/master/fetch_systems_repo.dart';
...
class FetchSystemsBloc extends Bloc<FetchSystemsEvent, FetchSystemsState> {
  ...
  Future<void> _onGetFetchSystemApi(
    OnFetchSystemsOfPortions event,
    Emitter<FetchSystemsState> emit,
  ) async {
    ...
    if (snapshot['error'] == false) {
      ...
      final list = data.map((e) => SystemModel.fromJson(e)).toList();
      list.sort(
        (a, b) => int.parse(a.position).compareTo(int.parse(b.position)),
      );
      emit(SuccessFetchSystemsState(listOfSystemes: list));
